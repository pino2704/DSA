...
public class AdjacencyMatrix {
    ...
    public static void main(String[] args) {
        AdjacencyMatrix temp = new AdjacencyMatrix(0);

        AdjacencyMatrix am = temp.readGraph("matrix.txt");

        am.printGraph();

        int[][] amArr = am.getAdjacencyMatrix();

        int countVertex = 0;

        for(int i = 0; i < am.NUMBER_OF_VERTICES; i++) {
            for(int j = 0; j < am.NUMBER_OF_VERTICES; j++) {
                if(amArr[i][j] == 1)    countVertex++;
            }
        }

        //Ex 1b
        System.out.println("Number of vertices: " + (countVertex/2 - 1));

        //Ex 1c
        System.out.println("Number of edge: " + (countVertex/2));

        //Ex 1d
        am.enumerateNeighbors(1, 1);

        //Ex 1e
        System.out.println(am.isLinked(1, 2, 1));

        //Ex 4
        AdjacencyList al = am.convertToAL();
        al.printGraph();
